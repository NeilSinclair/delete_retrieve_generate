...
for set_ in datasets:
    ...
    x0 = pd.read_csv(f'./data/{d[0]}/{d[0]}_{set_}_{d[1]}.0')
    x1 = pd.read_csv(f'./data/{d[0]}/{d[0]}_{set_}_{d[1]}.1')
    for col in x0.columns:
        ...
        if d[0] == 'yelp' and set_=='test':
            x0.sample(frac=0.02).to_csv(f'./data/{d[0]}/{d[0]}_{set_}_short_{d[1]}.0', index=False)
            x1.sample(frac=0.02).to_csv(f'./data/{d[0]}/{d[0]}_{set_}_short_{d[1]}.1', index=False)

        x0.to_csv(f'./data/{d[0]}/{d[0]}_{set_}_{d[1]}.0', index=False)
        x1.to_csv(f'./data/{d[0]}/{d[0]}_{set_}_{d[1]}.1', index=False)
...
new_df.to_csv(f'./data/{d[0]}/{d[0]}_train_{d[1]}.corpus', index=False)